...
#script d'installation et configuration d'un serveur minecraft spigot sous Debian.
VERT="\\033[1;32m"
ROUGE="\\033[1;31m"
NORMAL="\\033[0;39m"
CYAN="\\033[1;36m"
...
while [ $ver = "list" ] ; do
	...
	echo -e "$CYAN" " versions disponibles : 1.8.8 \n 1.8.7 \n 1.8.6\n et latest (pour la dernière en date)\n  Entrez la version désirée :" "$NORMAL"
	read ver
	break

done
read -p "Installer java/git ? (yes/no)" javach
if [ $javach = "yes" || $javach = "y" ] ; then 
echo -e "Installation de open-jre :"
apt-get update
apt-get install openjdk-7-jre openjdk-7-jdk vim git 
sed -nre 's/"syntax/syntax' /etc/vim/vimrc
fi
echo -e "$ROUGE" "Téléchargement de BuildTools.jar" "$VERT"
...
mv BuildTools.jar /srv/minecraft_server ; cd /srv/minecraft_server/
...
echo "alias "$aliass"='cd /srv/minecraft_server/ ; java -jar -Xms"$javamin" -Xmx"$javamax" spigot-"$ver".jar'" >> ~/.bashrc ; echo -e "$ROUGE"
#Compilation de BuildTools.jat
echo -e "$CYAN" "Téléchargement de Spigot... (may take a long time)\n"
